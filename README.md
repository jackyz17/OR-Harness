...
- **Two-layer memory**: an append-only **Execution Evidence Bank** (episodic facts: actual strategy/quality/cost, failures, artifacts) and a derived **Strategic Knowledge Bank** (commitments: expected quality/cost/failure risk, with prediction intervals, forward validation, and provenance citations; fully rebuildable from evidence). Statistics are computed on the fly, never persisted.
