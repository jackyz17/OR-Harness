...
`my-method` 刻意是框架从未见过的名字：框架不内置任何策略目录，因此不存在"是否属于目录"这个问题。`recall` 只报告你点名的方案在真实记忆里有什么（初始状态下是"空 + 原因"），`predict`/`execute` 接受你提出的任何方案。完整走查（空库 → 外层提出方案 → 代价未知 → 执行 → 答案校验 → 记录 → 召回）是一个可运行脚本：`PYTHONPATH=src python3 references/examples/no_catalog.py`。
