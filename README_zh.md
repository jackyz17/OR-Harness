...
- **双层记忆**：append-only 的 **Execution Evidence Bank**（执行证据层：实际策略/实际质量/实际代价/失败与恢复/产物）+ 派生的 **Strategic Knowledge Bank**（战略知识层：期望质量/期望代价/期望风险,带预测区间、前瞻校验与出处引用,可全量重建）。条件统计即时计算、永不落库。
