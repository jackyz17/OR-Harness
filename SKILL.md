...
- **Two units, never conflated.** Occurrence rates count real OBSERVATION UNITS (an execution counted once, however many predictions were bound to it); Brier/probability statistics count prediction-observation PAIRS. Only `scored_occurrence_rate` shares a denominator with `mean_predicted_probability`.
