...
- **Strategic Knowledge Bank** — induced commitments ("what to do next time": expected quality, expected cost, expected failure risk): prediction intervals, calibration tracking, applicability predicates read off the supporting evidence (family + the feature span it covered). Mutation happens at INDUCTION time only: recording collects evidence, the next `induce` creates/refreshes/revises entries. Creating one takes ≥2 supporting executions from ≥2 distinct tasks (repetitions of one task are not reproduction). Admission never depends on the survival of the original evidence rows, and `induce --rebuild` re-induces from currently retained evidence (exact reconstruction is not a requirement).
...
- **group / evidence set** — one (family, strategy) cell: the observations that may be aggregated together. What a claim covers inside that family is its own business (the intervals read off its evidence), not a binning rule.
...
### `orx induce [--strategy S | --all] [--rebuild] [--dry-run] [--force] [--note TEXT]`

Consolidates facts into Strategic Knowledge (your explicit call — hints never auto-induce). This is the ONLY place knowledge changes: it (i) builds or refreshes the claim of each (family, strategy) evidence set — its applicability is read off the supporting records (the family, and the span each structural dimension covered), so accumulating evidence is what moves it; and (ii) replays the frozen checks recorded on the facts, reporting what it did under `result.revisions` (promotion at ≥5 checks / ≥70% hits, demotion at 3 consecutive misses, dormancy wakeup). New entries are born `candidate` with honest intervals (width floored by sample size; n=2 cannot claim [0.95, 1.0]). One evidence set owns exactly one claim: re-inducing refreshes it rather than restating it. `--rebuild` re-induces the whole Strategic Knowledge Bank from currently retained evidence (cold archive preserved); the result may differ from the previous bank — exact reconstruction is not a requirement. New entries inherit `strategy_type`, `actions` and `fallback_strategy_id` from the catalog vocabulary (harness-supplied values are never overwritten).

**Independent evidence gate (creation only).** Creating a claim takes at least 2 supporting executions from ≥2 distinct `task_id`s: repeating one task is repetition, not reproduction. When only one task is behind the evidence the call creates nothing and returns `verification {tasks, required_tasks}` plus a `skipped` reason — the evidence is NOT discarded (recall still answers from it as `conditional_stats`), and refreshing an entry that already exists is never blocked, no matter how repetitive the new evidence is.

Each element of `result.results` reports `created` (entry id) / `updated` (entry id) / `skipped` (human-readable reason: fewer than 2 executions, needs independent evidence, cold-archive veto, restatement) — read the reason, it tells you what the memory is still missing.
...
- **induction_hints after record**: hints are evidence, not orders. C1 (significant contrast between ≥2 strategies), C2 (extreme high/low performance with n≥2), C3 (in-group drift), C4 (fallback exercised), C5 (same-direction performance reproduced in ≥2 families), C6 (stable success). Induce when you judge the pattern worth generalizing; you may also induce with no hint at all.
- **status**: `candidate` = plausible, unproven. `validated` = ≥5 frozen checks with ≥70% hits. `suspect` = 3 consecutive content misses, downweighted ×0.5 — treat its estimates as warnings, not facts. `dormant` = not consulted for 10 tasks, excluded from matching (the next induction wakes it if new matching evidence arrived). Promotion, demotion and dormancy wakeup are all applied by `orx induce`, never by `record`. Cost deviations never demote or re-scope an entry — they stay on the execution as `cost_feedback` evidence.
