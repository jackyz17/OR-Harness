...
- **A prediction input must be frozen, shared and provable.** Build the context ONCE per decision and reuse it across candidates (`plan-next` does this internally); a context or recall result from another task version / episode is REFUSED, not silently aligned. Replaying a stored context reads nothing from today's banks.
