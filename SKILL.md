...
- **One CIR per request.** An explicit `--cir` drives the joint representation, the snapshot's structural cell AND the retrieval; a supplied CIR replaces the task's own. Check `joint.sources.cir` — two structural judgments in one request means knowledge may be retrieved from the wrong cell.
