...
1. **Profile (the single analysis entry)** — submit your coupling understanding as the task JSON's optional `coupling` field (a CIR — Coupling-Aware Intermediate Representation) and run `orx profile --task t.json`. One call returns: the validated CIR with `modeling_guidance` (explicit, inspectable coupling groups), the problem profile, and the derivation report (each coupling dimension's value and origin). The profile serves two purposes: it helps YOU understand the problem structure, and it is the structural key that retrieves comparable evidence in the next step. **A task without a `model` field is a normal state** — strategy selection needs the task text, the CIR, and the profile, never a finished formulation. If the task carries a cost budget, declare it up front (`orx budget --task ID --declare llm_tokens=50000,...`) — the automatic pre-call budget guard protects only limits that were declared.
2. **Recall and compare strategies** — `orx recall --task t.json --top 3`. The profile is only the retrieval key: it decides WHICH evidence is comparable to this problem, not which strategy to pick. The choice itself rests on each candidate's expected quality, expected cost, and failure risk (`score = α·Q̂ − β·Ĉ − γ·R̂`), read from `evidence`, `confidence`, and `risk_warnings`; check `solver_advisories` for solvers that failed in this environment before. When there is no experience yet, candidates return with `evidence="no_memory"`, `score=-inf`, `confidence=0` — the catalog is a structural vocabulary (applicability, actions, fallback, solver family), not a source of fabricated priors. You may `--exclude` any candidate and re-recall. Retrieval today is an EXACT structural match (family + coupling cells) — there is no text or fuzzy search, so an empty recall means no evidence in THIS cell; a neighboring cell may hold evidence the key cannot reach, so never conclude "no experience anywhere" from one empty recall. **With a world model configured**: `orx plan-next` compares candidates by their PREDICTED consequences (same quality/cost/risk yardstick) and suggests a first step — use it for high-stakes decisions (expensive execution, tied candidates, unfamiliar cell). It predicts every candidate, but only the prediction of the path you actually execute gets bound (step 7); the rest served the comparison.
...
6. **Predict outcome (mandatory when a world model is configured)** — with a world model configured (`--world-model`), ask for a structured outcome prediction of the candidate execution (`orx predict-outcome`) BEFORE executing. It never changes your choice; you compare it afterwards (`orx bind-outcome`) to accumulate calibration evidence. Every executed prediction–comparison pair is the ONLY source of calibration evidence — skipping the prediction means the model can never be calibrated; this is not a place to save tokens. Skipping is legitimate ONLY when no world model is configured (the command then returns an explicit `not_configured`). If you already ran `plan-next` in step 2, do NOT predict again — bind the selected path's prediction in step 7; one executed action gets exactly one bound prediction.
7. **Execute** — `orx execute ...`. Every execution — successes AND failures — is automatically staged in a pending area (never lost, even if you immediately retry). Inspect `result.execution.quality.problems` before recording. Bind the prediction now — a prediction made but never bound is discarded evidence (`orx bind-outcome --prediction <id> --action <action_id from the execute output>`; with `plan-next`, the id of the SELECTED path's prediction). Bind failed executions too: a predicted failure that hit is calibration evidence.
8. **Verify** (see Verification below) — do not record an execution you have not checked. Report the verification with `orx action --report verify --task t.json` so later snapshots carry the evidence.
...
10. **Decide on induction** — hints are evidence, not orders. Induce only when you judge the pattern worth generalizing. When the task is done, close it with `orx action --report finish_task --task t.json`.
...
| World model configured + any task | `recall` for candidates, then `predict-outcome` the chosen candidate BEFORE executing and `bind-outcome` after — every execution feeds calibration; a prediction never bound is discarded evidence |
| World model configured + high-stakes decision (expensive execution, tied candidates, unfamiliar cell) | `plan-next` — compare candidates by PREDICTED consequences before choosing; bind the selected path's prediction, do not predict again |
...
**Cost discipline**: every world-model call spends real tokens (charged to the decision action / maintenance scope). `plan-next` and `assess-induction` are avoidable planning overhead — skip them when a plain `recall` already answers the question. `predict-outcome`/`bind-outcome` are NOT in that category: they collect calibration data (one pair per executed action) and are skipped only when no world model is configured.
...
| `orx [--world-model URL::MODEL] predict-outcome / bind-outcome` | Shadow prediction of one candidate action; bind + compare after execution (a required pair per executed action when configured) |
...
- Do not skip `predict-outcome`/`bind-outcome` when a world model is configured — calibration evidence cannot be backfilled; a prediction made but never bound is discarded evidence.
- Do not run `predict-outcome` again after a `plan-next` selection — bind the selected path's prediction; one executed action gets exactly one bound prediction.
- Do not leave your own actions (model / select_strategy / verify / finish_task) unreported — they enter the unified action log only via `orx action --report`; unreported work starves the world model's state and hides real token spend from the budget view.
