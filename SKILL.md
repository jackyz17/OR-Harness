...
- **Strategic Knowledge Bank** — induced commitments ("what to do next time": expected quality, expected cost, expected failure risk): prediction intervals, calibration tracking, feature predicates. **Target** (migration pending, next Induction round): candidates complete admission validation in offline induction; online execution only records new evidence; the next induction revises knowledge. **Current**: entries are born `candidate` and forward prediction checks promote/demote/tighten online. In either case, admission never depends on the survival of the original evidence rows, and `induce --rebuild` re-induces from currently retained evidence (exact reconstruction is not a requirement).
...
Appends the fact to the Execution Evidence Bank, then runs the automatic chain: cost backfill (replace by default — idempotent, never double-counts) → quality prediction checks against matching entries (hits/misses feed calibration; 3 consecutive QUALITY misses demote an entry to `suspect`; cross-family misses tighten an L2/L3 entry's scope) → cost feedback against the record's frozen pre-execution prediction snapshot (same strategy, same scope, both sides measured; written into `execution_features.cost_feedback` as Execution Evidence — it NEVER mutates entry state or calibration online; a later `update_cost` re-computes it) → C1–C6 induction-hint checks (C4 detects cross-execution recovery chains automatically: a failed attempt under one solver followed by success under another).
