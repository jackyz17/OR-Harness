...
| `orx [--world-model URL::MODEL] predict-outcome / bind-outcome` | M2 shadow prediction of one candidate action; bind + compare after execution |
| `orx [--world-model URL::MODEL] plan-next / choose-next` | M3 bounded planning by predicted consequences; record your explicit choice |
| `orx [--world-model URL::MODEL] assess-induction` | M4 offline induction value assessment (never induces by itself) |
